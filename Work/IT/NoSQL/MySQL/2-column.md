...
# 长度约束
* mysql表内 字段的字符长度累加不能超过65535，varchar按utf8mb4 ≈16383
* MySQL5.6 对单个预处理语句的占位符数量有严格限制，通常为 65,536 个（16 位无符号整数最大值） ——注意版本差异
> 批量插入：若插入 m 列、n 行数据，需满足 m × n < 65,536
> 查询条件过多：如 IN 子句中包含数万项（SELECT * FROM table WHERE id IN (?, ?, ...)）
