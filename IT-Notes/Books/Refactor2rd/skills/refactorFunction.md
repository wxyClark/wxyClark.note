...
## 4内联变量（Inline Variable）   

| title | content |
| ---- | ---- |
| 场景 | 有一个临时变量，只被一个简单表达式赋值一次，而它妨碍了其他重构手法。 |
| 意义 | 简化逻辑 |
| 定义 |将所有对该变量的引用动作，替换为对它赋值的那个表达式自身。  |
...
| 注意 | 多个内联变量在一起，可以用新函数取代临时变量 |

## 5改变函数声明（Change Function Declaration）   

修改函数名：在确保不重复的情况下，新函数使用旧的函数名，将旧的函数名添加Old后缀，测试函数的行为没有改变后，修改新函数名称，替换旧函数的调用

| title | content |
| ---- | ---- |
| 场景 | 函数的名称未能揭示函数的用途。修改函数的名称。 |
